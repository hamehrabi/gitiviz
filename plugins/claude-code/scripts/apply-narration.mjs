...
var GitError = class extends Error {
  ...
  constructor(opts) {
    super(
      `git ${opts.args.join(" ")} failed in ${opts.repoDir} (exit ${opts.exitCode ?? "signal"}): ${opts.stderr.trim()}`
    );
    this.name = "GitError";
...
// packages/git/src/refs.ts
var SHA40 = /^[0-9a-f]{40}$/;
async function resolveRef(repoDir, ref) {
  let stdout;
  try {
    ...
    throw new Error(
      `Unknown git ref "${ref}" in repository ${repoDir}. Expected a branch, tag, or commit sha that exists locally.`,
      { cause }
    );
  }
...
async function blobAt(repoDir, sha, path) {
  try {
    ...
    return SHA402.test(blob) ? blob : void 0;
  } catch {
    return void 0;
  }
}
async function diffRange(repoDir, baseRef, headRef) {
  ...
  return Promise.all(
    parseNameStatusZ(stdout).map(async (record) => {
      const change = { path: record.path, status: record.status };
      if (record.oldPath !== void 0) change.oldPath = record.oldPath;
      if (record.status !== "added") {
        const blob = await blobAt(repoDir, baseSha, record.oldPath ?? record.path);
        if (blob !== void 0) change.baseBlob = blob;
      }
      if (record.status !== "deleted" && headSha !== null) {
        const blob = await blobAt(repoDir, headSha, record.path);
        if (blob !== void 0) change.headBlob = blob;
      }
      return change;
    })
  );
...
var MAX_ANALYZED_LINE_LENGTH = 2e3;

// packages/analyzers/src/packageAnalyzer.ts
var ANALYZER_ID = "js-package@1";
...
function anchorForKey(path, lines, key) {
  const needle = JSON.stringify(key);
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const capped = line.length > MAX_ANALYZED_LINE_LENGTH ? line.slice(0, MAX_ANALYZED_LINE_LENGTH) : line;
...
// packages/analyzers/src/importExportAnalyzer.ts
var ANALYZER_ID2 = "js-import-export@1";
var JS_EXTENSIONS = /* @__PURE__ */ new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".mts", ".cts"]);
var MULTILINE_LOOKAHEAD = 40;
var FROM_SPECIFIER = /\bfrom\s+(?:"([^"\n]*)"|'([^'\n]*)')/;
var BARE_IMPORT = /^import\s+(?:"([^"\n]*)"|'([^'\n]*)')/;
var DYNAMIC_IMPORT_CALL = /\bimport\s*\(\s*/g;
var EXPORT_DECL = /^export\s+(?:default\s+)?(?:abstract\s+)?(?:async\s+)?(function|class|const|let|var|interface|type|enum)\s+\*?\s*([^\s(=:;,{<]+)/;
...
function fromSpecifierOf(line) {
  const m = FROM_SPECIFIER.exec(line);
  if (!m) return void 0;
  return m[1] ?? m[2];
}
function scanDynamicImports(line) {
  const specifiers = [];
  let nonLiteral = 0;
...
var importExportAnalyzer = {
  ...
  analyze(input) {
    ...
    if (sawDynamic || dynamicNonLiteral > 0) {
      const detail = dynamicNonLiteral > 0 ? `; ${dynamicNonLiteral} call(s) use a non-literal specifier and were not resolved` : "";
      limitations.push({
    ...
    if (sawReexport) {
      limitations.push({
        message: "Re-export detection is best-effort: recorded as imports of the source module; re-exported names are not tracked (no AST parse in v0.1)",
        path,
        analyzer: ANALYZER_ID2
      });
    }
...
// packages/analyzers/src/routeAnalyzer.ts
var ANALYZER_ID3 = "js-express-route@1";
var JS_EXTENSIONS2 = /* @__PURE__ */ new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".mts", ".cts"]);
var ROUTE_CALL = /\b(app|router)\s*\.\s*(get|post|put|patch|delete|head|options|all)\s*\(\s*/g;
...
var routeAnalyzer = {
  id: ANALYZER_ID3,
  appliesTo(path) {
    return JS_EXTENSIONS2.has(extensionOf2(path));
  },
  analyze(input) {
    const facts = [];
    const limitations = [];
...
function statesOf(inBase, inHead, changed) {
  if (inBase && inHead) {
    return changed ? { baseState: "changed", headState: "changed" } : { baseState: "unchanged", headState: "unchanged" };
  }
  if (inHead) return { baseState: "removed", headState: "added" };
  return { baseState: "added", headState: "removed" };
}
function entityId(kind, technicalLabel) {
...
function stableValueString(value) {
  return Object.keys(value).sort().map((k) => `${k}=${value[k]}`).join(", ");
}
function factIdentityOf(fact) {
  const v = fact.value;
  switch (fact.kind) {
    case "route":
      if (v.method !== void 0 && v.path !== void 0) {
...
function buildEvidenceGraph(input) {
  ...
  const relationshipBuckets = /* @__PURE__ */ new Map();
  const addRelationship = (verb, from, to, side, evidence) => {
    ...
    if (evidence !== void 0) bucket.evidence.push(evidence);
  };
  const byBasePath = /* @__PURE__ */ new Map();
  const byHeadPath = /* @__PURE__ */ new Map();
  const fileEntityIds = /* @__PURE__ */ new Map();
  const filePresence = /* @__PURE__ */ new Map();
  for (const fc of input.fileChanges) {
    ...
    if (inBase && basePath !== fc.path) {
      const baseAnchor = { path: basePath };
      if (fc.baseBlob !== void 0) baseAnchor.baseBlob = fc.baseBlob;
      evidence.push(baseAnchor);
      const headAnchor = { path: fc.path };
      if (fc.headBlob !== void 0) headAnchor.headBlob = fc.headBlob;
      evidence.push(headAnchor);
    } else {
      const anchor = { path: fc.path };
      if (fc.baseBlob !== void 0) anchor.baseBlob = fc.baseBlob;
      if (fc.headBlob !== void 0) anchor.headBlob = fc.headBlob;
  ...
  for (const fc of input.fileChanges) {
    ...
    if (members === void 0) groups.set(dir, [fc]);
  ...
  for (const bucket of buckets.values()) {
    ...
    const contentOf = (facts) => facts.map((f) => stableValueString(f.value)).sort().join("\n");
    const changed = inBase && inHead && contentOf(bucket.base) !== contentOf(bucket.head);
    const id = entityId(kind, technicalLabel);
    const evidence = [];
    const anchorFor = (fact, side) => {
      const anchor = {
        path: fact.anchor.path,
        range: { startLine: fact.anchor.startLine, endLine: fact.anchor.endLine }
      };
      const fc = side === "base" ? byBasePath.get(fact.anchor.path) : byHeadPath.get(fact.anchor.path);
      if (fc !== void 0) {
        if (side === "base" && fc.baseBlob !== void 0) anchor.baseBlob = fc.baseBlob;
        if (side === "head" && fc.headBlob !== void 0) anchor.headBlob = fc.headBlob;
      }
      return anchor;
    };
    ...
    evidence.sort(
      (a, b) => a.path !== b.path ? a.path < b.path ? -1 : 1 : (a.range?.startLine ?? 0) - (b.range?.startLine ?? 0)
    );
    entities.set(id, {
      id,
      kind,
  ...
  const entityList2 = [...entities.values()].sort(
    (a, b) => a.kind !== b.kind ? a.kind < b.kind ? -1 : 1 : (a.technicalLabel ?? "") < (b.technicalLabel ?? "") ? -1 : (a.technicalLabel ?? "") > (b.technicalLabel ?? "") ? 1 : 0
  );
  const relationshipList2 = [...relationshipBuckets.values()].map((bucket) => {
    const rel = {
    ...
    if (bucket.evidence.length > 0) {
      rel.evidence = [...bucket.evidence].sort(
        (a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : (a.range?.startLine ?? 0) - (b.range?.startLine ?? 0)
      );
    }
    return rel;
  }).sort(
    (a, b) => a.verb !== b.verb ? a.verb < b.verb ? -1 : 1 : a.from !== b.from ? a.from < b.from ? -1 : 1 : a.to < b.to ? -1 : a.to > b.to ? 1 : 0
  );
  return { entities: entityList2, relationships: relationshipList2 };
}

// packages/core/src/changeUnits.ts
...
function parseLog(stdout) {
  ...
  for (const line of stdout.split("\n")) {
    ...
    if (sha === void 0 || parents === void 0) continue;
    commits.push({
      sha,
...
async function classify(repoDir, commit) {
...
async function touchedPaths(repoDir, commit) {
  const parent = commit.parents[0] ?? EMPTY_TREE;
  const { stdout } = await gitRaw(repoDir, [
    "diff",
    "--name-status",
  ...
  for (const record of parseNameStatusZ(stdout)) {
    ...
    if (record.oldPath !== void 0) paths.add(record.oldPath);
  }
  return paths;
}
function attachedEntityIds(entities, paths) {
  const ids = [];
...
async function buildChangeUnits(input) {
  ...
  if (input.headRef === WORKTREE) {
    limitations.push({
      message: "Uncommitted working-tree changes belong to no commit and are not part of any change unit."
    });
  }
  const { stdout } = await gitRaw(repoDir, [
    "log",
...
// spec/book-manifest.schema.json
var book_manifest_schema_default = {
  $schema: "http://json-schema.org/draft-07/schema#",
  $id: "https://gitiviz.dev/spec/book-manifest.schema.json",
  title: "Gitiviz Book Manifest",
  type: "object",
  required: ["specVersion", "repository", "chapters"],
  additionalProperties: false,
  properties: {
    specVersion: {
      type: "string",
    ...
    chapters: {
      ...
      allOf: [
        { contains: { type: "object", properties: { id: { const: "purpose" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "journeys" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "systems" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "capabilities" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "flows" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "contracts" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "security" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "operations" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "decisions" } }, required: ["id"] } },
        { contains: { type: "object", properties: { id: { const: "history" } }, required: ["id"] } }
      ]
    }
  },
  definitions: {
...
var change_manifest_schema_default = {
  $schema: "http://json-schema.org/draft-07/schema#",
  $id: "https://gitiviz.dev/spec/change-manifest.schema.json",
  title: "Gitiviz Change Manifest",
  type: "object",
  required: [
    "specVersion",
    "repository",
  ...
  properties: {
    ...
    changeUnits: {
      type: "array",
      items: { $ref: "#/definitions/changeUnit" }
    },
    analysisLimitations: {
      type: "array",
  ...
  definitions: {
    provenance: {
      type: "string",
      enum: ["declared", "derived", "inferred", "unknown"]
    },
    changeState: {
      type: "string",
    ...
    evidenceAnchor: {
      ...
      properties: {
        ...
        baseBlob: { type: "string", pattern: "^[0-9a-f]{40}$" },
        headBlob: { type: "string", pattern: "^[0-9a-f]{40}$" },
        range: { $ref: "#/definitions/sourceRange" },
        symbol: { type: "string" },
    ...
    entity: {
      type: "object",
      required: ["id", "kind", "humanLabel", "baseState", "headState", "provenance"],
      additionalProperties: false,
      properties: {
        id: { type: "string", minLength: 1 },
        kind: { type: "string", minLength: 1 },
        humanLabel: { type: "string", minLength: 1 },
        technicalLabel: { type: "string" },
        baseState: { $ref: "#/definitions/changeState" },
        headState: { $ref: "#/definitions/changeState" },
    ...
    relationship: {
      type: "object",
      required: ["id", "from", "to", "verb", "baseState", "headState", "provenance"],
      additionalProperties: false,
      properties: {
        id: { type: "string", minLength: 1 },
    ...
    changeUnit: {
      type: "object",
      required: ["id", "technicalTitle", "provenance"],
      additionalProperties: false,
      properties: {
        id: { type: "string", minLength: 1 },
        technicalTitle: { type: "string", minLength: 1 },
        humanTitle: { type: ["string", "null"] },
        type: { type: "string" },
        commits: {
          type: "array",
        ...
        groupedReason: { type: "string" },
        summary: { type: ["string", "null"] },
        beforeDescription: { type: ["string", "null"] },
        afterDescription: { type: ["string", "null"] },
...
var compiledChangeManifest = ajv.compile(change_manifest_schema_default);
var compiledBookManifest = ajv.compile(book_manifest_schema_default);
var SUPPORTED_SPEC_MAJOR = 0;
function formatErrors(validate) {
  return (validate.errors ?? []).map(
    (e) => `${e.instancePath === "" ? "/" : e.instancePath} ${e.message ?? "is invalid"}`
  );
}
function specVersionError(specVersion) {
...
function validateChangeManifest(input) {
...
function validateBookManifest(input) {
...
function buildBookManifest(manifest) {
  return {
...
function buildNarrationRequest(manifest) {
  const entities = manifest.entities.map((entity) => {
    ...
    if (entity.technicalLabel !== void 0) fact.technicalLabel = entity.technicalLabel;
    return fact;
  });
  const relationships = manifest.relationships.map(
    (rel) => ({
      id: rel.id,
      from: rel.from,
      to: rel.to,
  ...
  const changeUnits = manifest.changeUnits.map((unit) => {
    const fact = {
      id: unit.id,
    ...
    if (unit.groupedReason !== void 0) fact.groupedReason = unit.groupedReason;
    return fact;
  });
  return {
    specVersion: manifest.specVersion,
...
var CHANGE_UNIT_SLOTS = /* @__PURE__ */ new Set([
  "humanTitle",
  "summary",
  "beforeDescription",
  "afterDescription",
...
function checkRecord(raw, where, slots, allowedIds, seenIds, errors, diagramContext) {
  ...
  if (typeof id !== "string" || id.length === 0) {
    ...
    errors.push(
      `${where} references unknown id "${id.slice(0, 200)}" \u2014 allowed ids: ${truncatedIdList([...allowedIds])}`
    );
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    if (key === "openQuestions") {
      if (!Array.isArray(value) || value.length > MAX_OPEN_QUESTIONS || !value.every(
        (q) => typeof q === "string" && q.length <= MAX_NARRATION_LENGTH
      )) {
        errors.push(
          `${where}.openQuestions must be an array of at most ${MAX_OPEN_QUESTIONS} strings (each \u2264 ${MAX_NARRATION_LENGTH} chars)`
        );
        continue;
      }
...
function stampInferred(record, confidence) {
...
function applyNarration(manifest, response) {
  ...
  for (const record of validEntities) {
    const entity = entityById.get(record["id"]);
  ...
  for (const record of validUnits) {
    const unit = unitById.get(record["id"]);
...
function templateNarrator(request) {
  const changeUnits = [];
  for (const unit of request.changeUnits) {
    if (unit.grouped === true) continue;
    const count = unit.entities.length;
    const touched = count === 0 ? "It touches no tracked entities." : `It touches ${count} tracked ${count === 1 ? "entity" : "entities"}.`;
    changeUnits.push({
      id: unit.id,
...
function computeRanks(sortedEntities, sortedRelationships) {
...
function routeEdge(relationship, from, to) {
  ...
  const [startX, endX] = fromCx < toCx ? [from.x + from.width, to.x] : [from.x, to.x + to.width];
  return {
    relationship,
    points: [
      { x: startX, y },
...
function layoutGraph(entities, relationships) {
  const sortedEntities = [...entities].sort(byId);
  const known = new Set(sortedEntities.map((entity) => entity.id));
  const sortedRelationships = relationships.filter(
    (relationship) => known.has(relationship.from) && known.has(relationship.to) && relationship.from !== relationship.to
  ).sort(byId);
  ...
  const height = 2 * PADDING + rows.length * NODE_HEIGHT + Math.max(0, rows.length - 1) * V_GAP;
  const nodes = [];
  ...
  const edges = sortedRelationships.map(
    (relationship) => routeEdge(relationship, nodeById.get(relationship.from), nodeById.get(relationship.to))
  );
  return { nodes, edges, width, height };
}

// packages/renderer/src/diagram.ts
var ACCENT = "#1d4ed8";
var INK = "#1f2937";
var MUTED = "#6b7280";
var BORDER = "#d1d5db";
var BORDER_STRONG = "#9ca3af";
var SURFACE = "#f9fafb";
var WHITE = "#ffffff";
...
function svgRoot(width, height, label, body) {
  return `<svg viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" role="img" aria-label="${escAttr(label)}" style="max-width:100%;height:auto" font-family="${FONT}"><title>${escHtml(label)}</title>` + body + `</svg>`;
}
function nodeSvg(node, style) {
  const { entity } = node;
  const cx = node.x + node.width / 2;
  const fullTitle = entity.technicalLabel ? `${entity.humanLabel} \u2014 ${entity.technicalLabel}` : entity.humanLabel;
  let g = `<g${style.opacity !== void 0 ? ` opacity="${style.opacity}"` : ""}>`;
  g += `<title>${escHtml(fullTitle)}</title>`;
  g += `<rect x="${node.x}" y="${node.y}" width="${node.width}" height="${node.height}" rx="${style.rx ?? 8}" fill="${style.fill ?? WHITE}" stroke="${style.stroke}" stroke-width="${style.strokeWidth ?? 1.5}"` + (style.dash ? ` stroke-dasharray="${style.dash}"` : "") + `/>`;
  if (style.tag) {
    g += `<text x="${node.x + node.width - 8}" y="${node.y + 14}" text-anchor="end" font-size="9" fill="${style.tagFill ?? MUTED}">${escHtml(style.tag)}</text>`;
  }
  const labelY = entity.technicalLabel ? node.y + 30 : node.y + 37;
  g += `<text x="${cx}" y="${labelY}" text-anchor="middle" font-size="12" font-weight="600" fill="${style.labelFill ?? INK}"` + (style.struck ? ` text-decoration="line-through"` : "") + `>${escHtml(truncate(entity.humanLabel, LABEL_MAX))}</text>`;
  if (entity.technicalLabel) {
    g += `<text x="${cx}" y="${node.y + 47}" text-anchor="middle" font-size="10" fill="${MUTED}">${escHtml(truncate(entity.technicalLabel, SUBLABEL_MAX))}</text>`;
  }
  g += `</g>`;
  return g;
}
function arrowHead(prev, tip, fill) {
  const half = 5;
  const depth = 8;
  let a;
  let b;
...
function haloText(x, y, fill, fontSize, content) {
  return `<text x="${x}" y="${y}" text-anchor="middle" font-size="${fontSize}" fill="${fill}" paint-order="stroke" stroke="${WHITE}" stroke-width="3">` + content + `</text>`;
}
function edgeSvg(edge, style) {
  const points = edge.points;
  const d = points.map((p, i) => `${i === 0 ? "M" : "L"}${p.x} ${p.y}`).join(" ");
  let g = `<g${style.opacity !== void 0 ? ` opacity="${style.opacity}"` : ""}>`;
  g += `<path d="${d}" fill="none" stroke="${style.stroke}" stroke-width="1.5"` + (style.dash ? ` stroke-dasharray="${style.dash}"` : "") + `/>`;
...
function contextDiagram(entities, relationships, options = {}) {
  ...
  if (internal.length > 0) {
    ...
    boundary = `<g><rect x="${minX}" y="${minY}" width="${maxX - minX}" height="${maxY - minY}" rx="10" fill="${SURFACE}" stroke="${BORDER}" stroke-dasharray="4 4"/><text x="${minX + 10}" y="${maxY - 9}" font-size="10" fill="${MUTED}">` + escHtml(options.boundaryLabel ?? "System boundary") + `</text></g>`;
  ...
  const label = `Context diagram: ${personCount} people, ${systemCount} systems, ${externalCount} external systems, ${layout.edges.length} connections.`;
  const body = boundary + layout.edges.map((edge) => edgeSvg(edge, { stroke: BORDER_STRONG })).join("") + layout.nodes.map((node) => nodeSvg(node, contextNodeStyle(node.entity))).join("");
  return svgRoot(layout.width, height, label, body);
}
function changeNodeStyle(state) {
  switch (state) {
    case "added":
...
function changeEdgeStyle(state) {
  switch (state) {
    case "added":
...
function changeDiagram(entities, relationships, changeUnit) {
  const layout = layoutGraph(entities, relationships);
  const counts = {
    added: 0,
    changed: 0,
  ...
  const subject = changeUnit ? changeUnit.humanTitle ?? changeUnit.technicalTitle : "this change";
  const label = `Before and after: ${subject} \u2014 ${counts.added} new, ${counts.changed} changed, ${counts.removed} removed, ${counts.unchanged} unchanged.`;
  const body = layout.edges.map((edge) => edgeSvg(edge, changeEdgeStyle(edge.relationship.headState))).join("") + layout.nodes.map((node) => nodeSvg(node, changeNodeStyle(node.entity.headState))).join("");
...
function entityList(entities) {
  const items = entities.map((entity) => {
    ...
    if (entity.technicalLabel) {
      label += ` <span class="muted">\xB7 ${escHtml(entity.technicalLabel)}</span>`;
    }
...
function relationshipList(change) {
  if (change.relationships.length === 0) return "";
  const byId2 = new Map(change.entities.map((entity) => [entity.id, entity]));
  const items = change.relationships.map((rel) => {
    const from = byId2.get(rel.from)?.humanLabel ?? rel.from;
...
function chapterFold(chapter, change) {
  ...
  return `<details class="fold"><summary>${escHtml(chapter.title)}</summary>` + question + content + `</details>`;
}
function moreView(book, change) {
  const folds = MORE_CHAPTER_IDS.map(
    (id) => book.chapters.find((chapter) => chapter.id === id)
...
function shellCss() {
  ...
  return [
    ...
    `body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#1f2937;background:#f9fafb;line-height:1.6}`,
...
async function blobContent(repoDir, blobSha) {
  const { stdout } = await gitRaw(repoDir, ["cat-file", "blob", blobSha]);
  return stdout;
}
function analyzeOneSide(path, content, into, limitations) {
  for (const analyzer of ANALYZERS) {
    ...
    try {
      ...
      limitations.push({
        ...
        message: `Analyzer ${analyzer.id} failed on ${path}: ${error instanceof Error ? error.message : String(error)}`
      });
    }
  }
}
async function collectFacts(repoDir, fileChanges, headIsWorktree) {
  const baseFacts = [];
  const headFacts = [];
  const limitations = [];
  for (const fc of fileChanges) {
    const basePath = fc.oldPath ?? fc.path;
    if (fc.status !== "added" && fc.baseBlob !== void 0) {
      try {
        analyzeOneSide(basePath, await blobContent(repoDir, fc.baseBlob), baseFacts, limitations);
      } catch (error) {
        limitations.push({
          path: basePath,
          message: `Could not read base content of ${basePath}: ${error instanceof Error ? error.message : String(error)}`
        });
      }
    }
    if (fc.status !== "deleted") {
      try {
        ...
        limitations.push({
          path: fc.path,
          message: `Could not read head content of ${fc.path}: ${error instanceof Error ? error.message : String(error)}`
        });
      }
    }
  }
...
async function runCompare(options) {
  ...
  try {
    ...
    if (error.code !== "ENOENT") {
      throw new Error(
        `${responsePath} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
function expectArgs(command, rest, min, max) {
  if (rest.length < min || rest.length > max) {
    const expected = min === max ? `${min}` : max === Number.MAX_SAFE_INTEGER ? `at least ${min}` : `${min}\u2013${max}`;
    throw new Error(
...
// packages/cli/src/apply-narration.ts
